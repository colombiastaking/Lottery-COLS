...
pub trait Lottery {
    ...
    fn start_lottery(
        ...
        self.total_tickets(&lottery_name).set(total_tickets); // Store total_tickets
    }
    ...
    #[endpoint(startNewRound)]
    fn start_new_round(&self, lottery_name: ManagedBuffer) {
        require!(
            self.blockchain().get_caller() == self.blockchain().get_owner_address(),
            "Only owner can call this!"
        );
        require!(
            !self.lottery_info(&lottery_name).is_empty(),
            "Lottery does not exist!"
        );
        let new_deadline = self.blockchain().get_block_timestamp() + 15 * 86_400;
        let total_tickets = self.total_tickets(&lottery_name).get();
        self.lottery_info(&lottery_name).update(|info| {
            info.deadline = new_deadline;
            info.tickets_left = total_tickets;
        });
        self.number_of_entries_for_user(&lottery_name, &self.blockchain().get_caller()).clear();
        ...
        self.new_round_started_event(&lottery_name, new_deadline);
    }

    #[event("newRoundStarted")]
    fn new_round_started_event(&self, #[indexed] lottery_name: &ManagedBuffer, #[indexed] new_deadline: u64);
    ...
    fn total_tickets(&self, lottery_name: &ManagedBuffer) -> SingleValueMapper<usize>;
}